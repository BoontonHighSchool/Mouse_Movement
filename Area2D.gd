...
func _process(delta):
	...
	if shoot:
		...
		projectile.position = $Sprite/Position2D.global_position #use node for shoot position
...
func _input(event):
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT and event.pressed:
		$Tween.interpolate_property($Sprite, "global_position",$Sprite.global_position, get_global_mouse_position(), 2,Tween.TRANS_LINEAR,Tween.EASE_IN_OUT)
		$Tween.start()
